...
function calcAzEl(output, T, localtime, latitude, longitude, zone) {
    ...
    if (output) {
        console.log('eqtbox ' + Math.floor(eqTime * 100 + 0.5) / 100.0);
        console.log('sdbox ' + Math.floor(theta * 100 + 0.5) / 100.0);
    }
...
function calcSolNoon(jd, longitude, timezone) {
    ...
    console.log(timeString(solNoonLocal, 3));
}
...
function calculate(lat, lng, today) {
    var times = {sunrise:today.clone().startOf('day'),sunset:today.clone().startOf('day')};
    var jday = getJD(today);
    var tl = getTimeLocal(today);
    var tz = today.utcOffset() / 60;
